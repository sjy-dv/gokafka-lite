...
func (m *CID) MarshalToSizedBufferVTStrict(dAtA []byte) (int, error) {
	...
	if m.PID != nil {
		size, err := m.PID.MarshalToSizedBufferVTStrict(dAtA[:i])
		if err != nil {
			return 0, err
		}
		i -= size
...
func (m *CID) UnmarshalVTUnsafe(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 1:
			...
			if err := m.PID.UnmarshalVTUnsafe(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 2:
...
func (m *MembersJoin) UnmarshalVTUnsafe(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
