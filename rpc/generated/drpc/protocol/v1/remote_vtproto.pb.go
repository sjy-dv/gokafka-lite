...
func (c *remoteClient) Receive(ctx context.Context, opts ...grpc.CallOption) (Remote_ReceiveClient, error) {
	stream, err := c.cc.NewStream(ctx, &Remote_ServiceDesc.Streams[0], "/gokafkalite.v1.Remote/Receive", opts...)
	if err != nil {
		return nil, err
	}
...
func (m *Envelope) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.Senders) > 0 {
		for iNdEx := len(m.Senders) - 1; iNdEx >= 0; iNdEx-- {
			size, err := m.Senders[iNdEx].MarshalToSizedBufferVT(dAtA[:i])
			if err != nil {
				return 0, err
			}
			i -= size
...
func (m *Envelope) MarshalToSizedBufferVTStrict(dAtA []byte) (int, error) {
	...
	if len(m.Senders) > 0 {
		for iNdEx := len(m.Senders) - 1; iNdEx >= 0; iNdEx-- {
			size, err := m.Senders[iNdEx].MarshalToSizedBufferVTStrict(dAtA[:i])
			if err != nil {
				return 0, err
			}
			i -= size
...
func (m *Envelope) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 2:
			...
			m.Targets = append(m.Targets, &PID{})
		...
		case 3:
			...
			m.Senders = append(m.Senders, &PID{})
			if err := m.Senders[len(m.Senders)-1].UnmarshalVT(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 4:
...
func (m *Envelope) UnmarshalVTUnsafe(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 3:
			...
			m.Senders = append(m.Senders, &PID{})
			if err := m.Senders[len(m.Senders)-1].UnmarshalVTUnsafe(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 4:
