...
func (s *SelfManaged) Receive(c *actor.Context) {
	...
	case *pb.Handshake:
		...
		s.cluster.engine.Send(c.Sender(), &pb.Members{
			Members: members,
		})
	case *pb.Members:
...
func (s *SelfManaged) handleMemberPing(c *actor.Context) {
	s.members.ForEach(func(member *pb.Member) bool {
...
func (s *SelfManaged) addMembers(members ...*pb.Member) {
...
func (s *SelfManaged) removeMember(member *pb.Member) {
	if s.members.Contains(member) {
...
func (s *SelfManaged) start(c *actor.Context) {
	...
	for _, member := range s.config.bootstrapMembers {
		...
		s.cluster.engine.SendWithSender(memberPID.PID, &pb.Handshake{
	...
	go func(results <-chan *zeroconf.ServiceEntry) {
		for entry := range results {
			if entry.Instance != s.cluster.ID() {
				...
				s.cluster.engine.SendWithSender(memberPID.PID, hs, self.PID)
			}
		}
